...
void LCD_Show_StandbyPage(void) //显示"待机页面"-"彩屏LCD_ILI9325"
{
	uint8_t  ChNum = 0;
		...
		//LCD_ILI9341_TFT_cls(White);  //全屏清屏-"彩屏LCD_ILI9325"
	...
	//	LCD_TFT_ShowString(74,48,"TI University Program",FONT1608,GreenYellow,Black);
		...
		yStart = 40;
		for(ChNum = 0;ChNum <= 7; ChNum++)//MSP430_FFTB,期末设计 温度报警
		{
			LCD_Show_ChFont2424(40,yStart,ChNum,Red,Green);  //LIGHTGREEN
			yStart += 26;
		}

		yStart = 40;
		for(ChNum = 8;ChNum <= 15; ChNum++) //同济大学电信学院
		{
			LCD_Show_ChFont2424(70,yStart,ChNum,Red,Green);
			yStart += 26;
		}

		yStart = 40;
		for(ChNum = 16;ChNum <= 21; ChNum++) //电子信息工程
		{
			LCD_Show_ChFont2424(100,yStart,ChNum,Red,Green);
			yStart += 26;
		}

		yStart = 40;
		for(ChNum = 22;ChNum <= 24; ChNum++) //名字
		{
			LCD_Show_ChFont2424(130,yStart,ChNum,Red,Green);
			yStart += 26;
		}

	//	LCD_TFT_ShowString(130,yStart,"2051518",FONT1608,Red,GreenYellow);
